...
local function getItemTooltipLines(slotInfo, class, forceAlt)
	...
	if class.BA.CO then
		...
		text1_lower = text1_lower .. TRP3_API.Colors.CRAFTING_REAGENT(PROFESSIONS_USED_IN_COOKING);
	end
...
local function showItemTooltip(frame, slotInfo, itemClass, forceAlt, anchor)
	...
	if text1 and text1:len() > 0 then
		local r, g, b = TRP3_API.Colors.ORANGE:GetRGB(); -- corresponds to color("o") = FFAA00 for the description
